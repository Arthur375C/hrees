class Hrees {
  ...
  // ================== Céu ==================
  addSky(color = 'skyblue') {
    this.sky = color;
    this.spriteSky = null;
    this.skyGradient = null;
  }

  addSpriteSky(imagePath) {
    const img = new Image();
    img.src = imagePath;
    this.spriteSky = img;
    this.sky = null;
  ...
  addGradientSky(topColor = '#87CEEB', bottomColor = '#ffffff') {
    this.skyGradient = { top: topColor, bottom: bottomColor };
    this.sky = null;
    this.spriteSky = null;
  }

  addMovingClouds(imagePath, speed = 0.5, yPos = 50) {
    const img = new Image();
    img.src = imagePath;
    img.onload = () => {
      for (let i = 0; i < 3; i++) {
        this.clouds.push({ img, x: i * 200, y: yPos + Math.random() * 50, speed });
      }
    };
  }
  ...
  addImage(path, options = {}) {
    const img = new Image();
    img.src = path;
    this.addObject({ type: 'image', image: img, width: 50, height: 50, x: 0, y: 0, ...options });
  }

  addText(text, options = {}) {
    this.addObject({ type: 'text', text, font: '20px Arial', color: 'black', x: 0, y: 0, ...options });
  }
  ...
  addTilemap(map, tileWidth = 50, tileHeight = 50, options = {}) {
    ...
    for (let y = 0; y < map.length; y++) {
      for (let x = 0; x < map[y].length; x++) {
        if (map[y][x]) {
          this.addRect(x * tileWidth, y * tileHeight, tileWidth, tileHeight, { color: 'grey', ...options });
        }
      }
    }
  }
  ...
  update() {
    ...
    for (let t of this.triggers) {
      for (let o of this.objects) {
        if (o.x + t.w > t.x && o.x < t.x + t.w &&
            o.y + t.h > t.y && o.y < t.y + t.h) {
          t.callback(o);
        }
      }
    }
  ...
  render(ctx) {
    ...
    // Câmera
    if (this.cameraTarget) ctx.translate(-this.cameraOffset.x, -this.cameraOffset.y);

    // Céu
    if (this.sky) {
      ctx.fillStyle = this.sky;
      ctx.fillRect(0, 0, ctx.canvas.width, ctx.canvas.height);
    } else if (this.skyGradient) {
      const grad = ctx.createLinearGradient(0, 0, 0, ctx.canvas.height);
      grad.addColorStop(0, this.skyGradient.top);
      grad.addColorStop(1, this.skyGradient.bottom);
      ctx.fillStyle = grad;
      ctx.fillRect(0, 0, ctx.canvas.width, ctx.canvas.height);
    } else if (this.spriteSky && this.spriteSky.complete) {
      ctx.drawImage(this.spriteSky, 0, 0, ctx.canvas.width, ctx.canvas.height);
    }
    ...
    for (let cloud of this.clouds) {
      if (cloud.img.complete) {
        ...
        cloud.x += cloud.speed;
        if (cloud.x > ctx.canvas.width) cloud.x = -cloud.img.width;
      }
    }

    this.renderTilemaps(ctx); // Tilemaps
    ...
    for (let obj of this.objects) {
      ...
      else if (obj.type === 'circle') ctx.fillStyle = obj.color, ctx.beginPath(), ctx.arc(0, 0, obj.radius, 0, Math.PI*2), ctx.fill();
